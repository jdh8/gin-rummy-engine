...
#[derive(Serialize)]
struct HintJson {
    /// The move label, e.g. `"discard 4♠"`, `"knock"`, `"take 4♠"`.
    action: String,
...
/// Reshape the solver's assessments into the serializable rows the page reads.
fn hint_json(assessments: Vec<Assessment>) -> Vec<HintJson> {
    assessments
        .into_iter()
        .map(|a| HintJson {
            action: a.action,
